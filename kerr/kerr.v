...
// The entries repeat the named consts as literals: V cannot statically
// initialize a fixed-array const from references to other struct consts
// (they are runtime-initialized), but literals fold into a static table.
pub const all_errors = [
	KafkaError{'UNKNOWN_SERVER_ERROR', -1, false, 'The server experienced an unexpected error when processing the request.'},
	KafkaError{'OFFSET_OUT_OF_RANGE', 1, false, 'The requested offset is not within the range of offsets maintained by the server.'},
	KafkaError{'CORRUPT_MESSAGE', 2, true, 'This message has failed its CRC checksum, exceeds the valid size, has a null key for a compacted topic, or is otherwise corrupt.'},
	KafkaError{'UNKNOWN_TOPIC_OR_PARTITION', 3, true, 'This server does not host this topic-partition.'},
	KafkaError{'INVALID_FETCH_SIZE', 4, false, 'The requested fetch size is invalid.'},
	KafkaError{'LEADER_NOT_AVAILABLE', 5, true, 'There is no leader for this topic-partition as we are in the middle of a leadership election.'},
	KafkaError{'NOT_LEADER_FOR_PARTITION', 6, true, 'This server is not the leader for that topic-partition.'},
	KafkaError{'REQUEST_TIMED_OUT', 7, true, 'The request timed out.'},
	KafkaError{'BROKER_NOT_AVAILABLE', 8, true, 'The broker is not available.'},
	KafkaError{'REPLICA_NOT_AVAILABLE', 9, true, 'The replica is not available for the requested topic-partition.'},
	KafkaError{'MESSAGE_TOO_LARGE', 10, false, 'The request included a message larger than the max message size the server will accept'},
	KafkaError{'STALE_CONTROLLER_EPOCH', 11, false, 'The controller moved to another broker.'},
	KafkaError{'OFFSET_METADATA_TOO_LARGE', 12, false, 'The metadata field of the offset request was too large.'},
	KafkaError{'NETWORK_EXCEPTION', 13, true, 'The server disconnected before a response was received.'},
	KafkaError{'COORDINATOR_LOAD_IN_PROGRESS', 14, true, "The coordinator is loading and hence can't process requests."},
	KafkaError{'COORDINATOR_NOT_AVAILABLE', 15, true, 'The coordinator is not available.'},
	KafkaError{'NOT_COORDINATOR', 16, true, 'This is not the correct coordinator.'},
	KafkaError{'INVALID_TOPIC_EXCEPTION', 17, false, 'The request attempted to perform an operation on an invalid topic.'},
	KafkaError{'RECORD_LIST_TOO_LARGE', 18, false, 'The request included message batch larger than the configured segment size on the server.'},
	KafkaError{'NOT_ENOUGH_REPLICAS', 19, true, 'Messages are rejected since there are fewer in-sync replicas than required.'},
	KafkaError{'NOT_ENOUGH_REPLICAS_AFTER_APPEND', 20, true, 'Messages are written to the log, but to fewer in-sync replicas than required.'},
	KafkaError{'INVALID_REQUIRED_ACKS', 21, false, 'Produce request specified an invalid value for required acks.'},
	KafkaError{'ILLEGAL_GENERATION', 22, false, 'Specified group generation id is not valid.'},
	KafkaError{'INCONSISTENT_GROUP_PROTOCOL', 23, false, "The group member's supported protocols are incompatible with those of existing members or first group member tried to join with empty protocol type or empty protocol list."},
	KafkaError{'INVALID_GROUP_ID', 24, false, 'The configured groupID is invalid.'},
	KafkaError{'UNKNOWN_MEMBER_ID', 25, false, 'The coordinator is not aware of this member.'},
	KafkaError{'INVALID_SESSION_TIMEOUT', 26, false, 'The session timeout is not within the range allowed by the broker (as configured by group.min.session.timeout.ms and group.max.session.timeout.ms).'},
	KafkaError{'REBALANCE_IN_PROGRESS', 27, false, 'The group is rebalancing, so a rejoin is needed.'},
	KafkaError{'INVALID_COMMIT_OFFSET_SIZE', 28, false, 'The committing offset data size is not valid.'},
	KafkaError{'TOPIC_AUTHORIZATION_FAILED', 29, false, 'Not authorized to access topics: [Topic authorization failed.]'},
	KafkaError{'GROUP_AUTHORIZATION_FAILED', 30, false, 'Not authorized to access group: Group authorization failed.'},
	KafkaError{'CLUSTER_AUTHORIZATION_FAILED', 31, false, 'Cluster authorization failed.'},
	KafkaError{'INVALID_TIMESTAMP', 32, false, 'The timestamp of the message is out of acceptable range.'},
	KafkaError{'UNSUPPORTED_SASL_MECHANISM', 33, false, 'The broker does not support the requested SASL mechanism.'},
	KafkaError{'ILLEGAL_SASL_STATE', 34, false, 'Request is not valid given the current SASL state.'},
	KafkaError{'UNSUPPORTED_VERSION', 35, false, 'The version of API is not supported.'},
	KafkaError{'TOPIC_ALREADY_EXISTS', 36, false, 'Topic with this name already exists.'},
	KafkaError{'INVALID_PARTITIONS', 37, false, 'Number of partitions is below 1.'},
	KafkaError{'INVALID_REPLICATION_FACTOR', 38, false, 'Replication factor is below 1 or larger than the number of available brokers.'},
	KafkaError{'INVALID_REPLICA_ASSIGNMENT', 39, false, 'Replica assignment is invalid.'},
	KafkaError{'INVALID_CONFIG', 40, false, 'Configuration is invalid.'},
	KafkaError{'NOT_CONTROLLER', 41, true, 'This is not the correct controller for this cluster.'},
	KafkaError{'INVALID_REQUEST', 42, false, 'This most likely occurs because of a request being malformed by the client library or the message was sent to an incompatible broker. See the broker logs for more details.'},
	KafkaError{'UNSUPPORTED_FOR_MESSAGE_FORMAT', 43, false, 'The message format version on the broker does not support the request.'},
	KafkaError{'POLICY_VIOLATION', 44, false, 'Request parameters do not satisfy the configured policy.'},
	KafkaError{'OUT_OF_ORDER_SEQUENCE_NUMBER', 45, false, 'The broker received an out of order sequence number.'},
	KafkaError{'DUPLICATE_SEQUENCE_NUMBER', 46, false, 'The broker received a duplicate sequence number.'},
	KafkaError{'INVALID_PRODUCER_EPOCH', 47, false, 'Producer attempted an operation with an old epoch.'},
	KafkaError{'INVALID_TXN_STATE', 48, false, 'The producer attempted a transactional operation in an invalid state.'},
	KafkaError{'INVALID_PRODUCER_ID_MAPPING', 49, false, 'The producer attempted to use a producer id which is not currently assigned to its transactional id.'},
	KafkaError{'INVALID_TRANSACTION_TIMEOUT', 50, false, 'The transaction timeout is larger than the maximum value allowed by the broker (as configured by transaction.max.timeout.ms).'},
	KafkaError{'CONCURRENT_TRANSACTIONS', 51, false, 'The producer attempted to update a transaction while another concurrent operation on the same transaction was ongoing.'},
	KafkaError{'TRANSACTION_COORDINATOR_FENCED', 52, false, 'Indicates that the transaction coordinator sending a WriteTxnMarker is no longer the current coordinator for a given producer.'},
	KafkaError{'TRANSACTIONAL_ID_AUTHORIZATION_FAILED', 53, false, 'Transactional ID authorization failed.'},
	KafkaError{'SECURITY_DISABLED', 54, false, 'Security features are disabled.'},
	KafkaError{'OPERATION_NOT_ATTEMPTED', 55, false, 'The broker did not attempt to execute this operation. This may happen for batched RPCs where some operations in the batch failed, causing the broker to respond without trying the rest.'},
	KafkaError{'KAFKA_STORAGE_ERROR', 56, true, 'Disk error when trying to access log file on the disk.'},
	KafkaError{'LOG_DIR_NOT_FOUND', 57, false, 'The user-specified log directory is not found in the broker config.'},
	KafkaError{'SASL_AUTHENTICATION_FAILED', 58, false, 'SASL Authentication failed.'},
	KafkaError{'UNKNOWN_PRODUCER_ID', 59, false, "This exception is raised by the broker if it could not locate the producer metadata associated with the producerID in question. This could happen if, for instance, the producer's records were deleted because their retention time had elapsed. Once the last records of the producerID are removed, the producer's metadata is removed from the broker, and future appends by the producer will return this exception."},
	KafkaError{'REASSIGNMENT_IN_PROGRESS', 60, false, 'A partition reassignment is in progress.'},
	KafkaError{'DELEGATION_TOKEN_AUTH_DISABLED', 61, false, 'Delegation Token feature is not enabled.'},
	KafkaError{'DELEGATION_TOKEN_NOT_FOUND', 62, false, 'Delegation Token is not found on server.'},
	KafkaError{'DELEGATION_TOKEN_OWNER_MISMATCH', 63, false, 'Specified Principal is not valid Owner/Renewer.'},
	KafkaError{'DELEGATION_TOKEN_REQUEST_NOT_ALLOWED', 64, false, 'Delegation Token requests are not allowed on PLAINTEXT/1-way SSL channels and on delegation token authenticated channels.'},
	KafkaError{'DELEGATION_TOKEN_AUTHORIZATION_FAILED', 65, false, 'Delegation Token authorization failed.'},
	KafkaError{'DELEGATION_TOKEN_EXPIRED', 66, false, 'Delegation Token is expired.'},
	KafkaError{'INVALID_PRINCIPAL_TYPE', 67, false, 'Supplied principalType is not supported.'},
	KafkaError{'NON_EMPTY_GROUP', 68, false, 'The group is not empty.'},
	KafkaError{'GROUP_ID_NOT_FOUND', 69, false, 'The group id does not exist.'},
	KafkaError{'FETCH_SESSION_ID_NOT_FOUND', 70, true, 'The fetch session ID was not found.'},
	KafkaError{'INVALID_FETCH_SESSION_EPOCH', 71, true, 'The fetch session epoch is invalid.'},
	KafkaError{'LISTENER_NOT_FOUND', 72, true, 'There is no listener on the leader broker that matches the listener on which metadata request was processed.'},
	KafkaError{'TOPIC_DELETION_DISABLED', 73, false, 'Topic deletion is disabled.'},
	KafkaError{'FENCED_LEADER_EPOCH', 74, true, 'The leader epoch in the request is older than the epoch on the broker'},
	KafkaError{'UNKNOWN_LEADER_EPOCH', 75, true, 'The leader epoch in the request is newer than the epoch on the broker'},
	KafkaError{'UNSUPPORTED_COMPRESSION_TYPE', 76, false, 'The requesting client does not support the compression type of given partition.'},
	KafkaError{'STALE_BROKER_EPOCH', 77, false, 'Broker epoch has changed'},
	KafkaError{'OFFSET_NOT_AVAILABLE', 78, true, 'The leader high watermark has not caught up from a recent leader election so the offsets cannot be guaranteed to be monotonically increasing'},
	KafkaError{'MEMBER_ID_REQUIRED', 79, false, 'The group member needs to have a valid member id before actually entering a consumer group'},
	KafkaError{'PREFERRED_LEADER_NOT_AVAILABLE', 80, true, 'The preferred leader was not available'},
	KafkaError{'GROUP_MAX_SIZE_REACHED', 81, false, 'The consumer group has reached its max size'},
	KafkaError{'FENCED_INSTANCE_ID', 82, false, 'The broker rejected this static consumer since another consumer with the same group.instance.id has registered with a different member.id.'},
	KafkaError{'ELIGIBLE_LEADERS_NOT_AVAILABLE', 83, true, 'Eligible topic partition leaders are not available'},
	KafkaError{'ELECTION_NOT_NEEDED', 84, true, 'Leader election not needed for topic partition'},
	KafkaError{'NO_REASSIGNMENT_IN_PROGRESS', 85, false, 'No partition reassignment is in progress.'},
	KafkaError{'GROUP_SUBSCRIBED_TO_TOPIC', 86, false, 'Deleting offsets of a topic is forbidden while the consumer group is actively subscribed to it.'},
	KafkaError{'INVALID_RECORD', 87, false, 'This record has failed the validation on the broker and hence been rejected.'},
	KafkaError{'UNSTABLE_OFFSET_COMMIT', 88, true, 'There are unstable offsets that need to be cleared.'},
	KafkaError{'THROTTLING_QUOTA_EXCEEDED', 89, true, 'The throttling quota has been exceeded.'},
	KafkaError{'PRODUCER_FENCED', 90, false, 'There is a newer producer with the same transactionalId which fences the current one.'},
	KafkaError{'RESOURCE_NOT_FOUND', 91, false, 'A request illegally referred to a resource that does not exist.'},
	KafkaError{'DUPLICATE_RESOURCE', 92, false, 'A request illegally referred to the same resource twice.'},
	KafkaError{'UNACCEPTABLE_CREDENTIAL', 93, false, 'Requested credential would not meet criteria for acceptability.'},
	KafkaError{'INCONSISTENT_VOTER_SET', 94, false, 'Indicates that either the sender or recipient of a voter-only request is not one of the expected voters.'},
	KafkaError{'INVALID_UPDATE_VERSION', 95, false, 'The given update version was invalid.'},
	KafkaError{'FEATURE_UPDATE_FAILED', 96, false, 'Unable to update finalized features due to an unexpected server error.'},
	KafkaError{'PRINCIPAL_DESERIALIZATION_FAILURE', 97, false, 'Request principal deserialization failed during forwarding. This indicates an internal error on the broker cluster security setup.'},
	KafkaError{'SNAPSHOT_NOT_FOUND', 98, false, 'Requested snapshot was not found.'},
	KafkaError{'POSITION_OUT_OF_RANGE', 99, false, 'Requested position is not greater than or equal to zero, and less than the size of the snapshot.'},
	KafkaError{'UNKNOWN_TOPIC_ID', 100, true, 'This server does not host this topic ID.'},
	KafkaError{'DUPLICATE_BROKER_REGISTRATION', 101, false, 'This broker ID is already in use.'},
	KafkaError{'BROKER_ID_NOT_REGISTERED', 102, false, 'The given broker ID was not registered.'},
	KafkaError{'INCONSISTENT_TOPIC_ID', 103, true, "The log's topic ID did not match the topic ID in the request."},
	KafkaError{'INCONSISTENT_CLUSTER_ID', 104, false, 'The clusterId in the request does not match that found on the server.'},
	KafkaError{'TRANSACTIONAL_ID_NOT_FOUND', 105, false, 'The transactionalId could not be found.'},
	KafkaError{'FETCH_SESSION_TOPIC_ID_ERROR', 106, true, 'The fetch session encountered inconsistent topic ID usage.'},
	KafkaError{'INELIGIBLE_REPLICA', 107, false, 'The new ISR contains at least one ineligible replica.'},
	KafkaError{'NEW_LEADER_ELECTED', 108, false, 'The AlterPartition request successfully updated the partition state but the leader has changed.'},
	KafkaError{'OFFSET_MOVED_TO_TIERED_STORAGE', 109, false, 'The requested offset is moved to tiered storage.'},
	KafkaError{'FENCED_MEMBER_EPOCH', 110, false, 'The member epoch is fenced by the group coordinator. The member must abandon all its partitions and rejoin.'},
	KafkaError{'UNRELEASED_INSTANCE_ID', 111, false, 'The instance ID is still used by another member in the consumer group. That member must leave first.'},
	KafkaError{'UNSUPPORTED_ASSIGNOR', 112, false, 'The assignor or its version range is not supported by the consumer group.'},
	KafkaError{'STALE_MEMBER_EPOCH', 113, false, 'The member epoch is stale. The member must retry after receiving its updated member epoch via the ConsumerGroupHeartbeat API.'},
	KafkaError{'MISMATCHED_ENDPOINT_TYPE', 114, false, 'The request was sent to an endpoint of the wrong type.'},
	KafkaError{'UNSUPPORTED_ENDPOINT_TYPE', 115, false, 'This endpoint type is not supported yet.'},
	KafkaError{'UNKNOWN_CONTROLLER_ID', 116, false, 'This controller ID is not known'},
	KafkaError{'UNKNOWN_SUBSCRIPTION_ID', 117, false, 'Client sent a push telemetry request with an invalid or outdated subscription ID.'},
	KafkaError{'TELEMETRY_TOO_LARGE', 118, false, 'Client sent a push telemetry request larger than the maximum size the broker will accept.'},
	KafkaError{'INVALID_REGISTRATION', 119, false, 'The controller has considered the broker registration to be invalid.'},
	KafkaError{'TRANSACTION_ABORTABLE', 120, false, 'The server encountered an error with the transaction. The client can abort the transaction to continue using this transactional ID.'},
	KafkaError{'INVALID_RECORD_STATE', 121, false, 'The record state is invalid. The acknowledgement of delivery could not be completed.'},
	KafkaError{'SHARE_SESSION_NOT_FOUND', 122, false, 'The share session was not found.'},
	KafkaError{'INVALID_SHARE_SESSION_EPOCH', 123, false, 'The share session epoch is invalid.'},
	KafkaError{'FENCED_STATE_EPOCH', 124, false, 'The share coordinator rejected the request because the share-group state epoch did not match.'},
	KafkaError{'INVALID_VOTER_KEY', 125, false, "The voter key doesn't match the receiving replica's key."},
	KafkaError{'DUPLICATE_VOTER', 126, false, 'The voter is already part of the set of voters.'},
	KafkaError{'VOTER_NOT_FOUND', 127, false, 'The voter is not part of the set of voters.'},
	KafkaError{'INVALID_REGULAR_EXPRESSION', 128, false, 'The regular expression is not valid.'},
	KafkaError{'REBOOTSTRAP_REQUIRED', 129, false, 'Client metadata is stale. The client should rebootstrap to obtain new metadata.'},
	KafkaError{'STREAMS_INVALID_TOPOLOGY', 130, false, 'The supplied topology is invalid.'},
	KafkaError{'STREAMS_INVALID_TOPOLOGY_EPOCH', 131, false, 'The supplied topology epoch is invalid.'},
	KafkaError{'STREAMS_TOPOLOGY_FENCED', 132, false, 'The supplied topology epoch is outdated.'},
	KafkaError{'SHARE_SESSION_LIMIT_REACHED', 133, true, 'The limit of share sessions has been reached.'},
]!
